...
Requires: real images in ImageFolder format at /root/data/{dataset}/train/
"""
...
def find_sd_model():
    ...
    import glob
    patterns = [
        "/root/data/hf_cache/models--stable-diffusion-v1-5--stable-diffusion-v1-5/snapshots/*",
    ]
    for pat in patterns:
        paths = glob.glob(pat)
        if paths:
...
def load_class_names(dataset, class_file=None):
    ...
    data_dir = cfg.get("data_dir", "/root/data/" + dataset)
