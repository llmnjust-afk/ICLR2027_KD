...
def evaluate_fast(train_dir, val_dir, class_names, num_classes, device,
    ...
    # Lighting augmentation (AlexNet-style)
    eigval = torch.tensor([0.2175, 0.0188, 0.0045], device=device)
    eigvec = torch.tensor([
    ...
    for epoch in range(epochs):
        ...
        for start in range(0, n_train, batch_size):
            ...
            # Lighting noise (AlexNet-style)
            alpha = 0.1 * torch.randn(imgs.size(0), 3, 1, 1, device=device)
            lighting = torch.einsum('bi,ij->bj', alpha.squeeze(-1).squeeze(-1), eigvec) * eigval.unsqueeze(0)
            imgs = imgs + lighting.unsqueeze(-1).unsqueeze(-1)
            imgs = normalize(imgs)
