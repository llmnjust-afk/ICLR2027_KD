#!/bin/bash
# Phase 2 v2: ImageNette + ImageWoof on GPU 0 (fixed spec names)
set -uo pipefail
cd /root/ICLR2027_KD
...
gen_and_eval() {
    local SPEC=$1 IPC=$2 TAG=$3 BASE=$4 DATA_DIR=$5 VAL_DIR=$6 LAMBDA=$7
    ...
    if [ ! -d "$DIR" ] || [ "$(count_pngs "$DIR")" -lt "$NEEDED" ]; then
        echo "  Generating ${NEEDED} images into ${DIR}..."
...
echo "========================================"
echo "  Phase 2 v2: ImageNette + ImageWoof on GPU 0"
echo "  Started: $(date)"
echo "========================================"

# ImageNette (10 classes, IPC=10)
gen_and_eval nette 10 unguided results/sweep_nette $NETTE_DIR $NETTE_VAL unguided
gen_and_eval nette 10 lambda005 results/sweep_nette $NETTE_DIR $NETTE_VAL 0.05

# ImageWoof (10 classes, IPC=10)
gen_and_eval woof 10 unguided results/sweep_woof $WOOF_DIR $WOOF_VAL unguided
gen_and_eval woof 10 lambda005 results/sweep_woof $WOOF_DIR $WOOF_VAL 0.05

echo ""
echo "========================================"
echo "  Phase 2 v2 (GPU 0) ALL DONE — $(date)"
echo "========================================"
