...
All experiments use 3 random seeds (seeds 0, 1, 2) except where noted in table captions (IN-1K ConvNet-6 uses 2 seeds; some sensitivity analysis entries use 2 seeds). Population std is used for reported standard deviations; sample std (ddof=1) is used for $t$-tests.
...
\paragraph{LLM usage.}
Large language models were used for assistance with code debugging and manuscript editing. All experimental design, analysis, and scientific conclusions were conducted solely by the authors. No LLM was used for data generation, result fabrication, or scientific reasoning.
