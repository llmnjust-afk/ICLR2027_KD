...
\midrule
Mode count only & 1.0 & 0.0 & 0.0 & 0.0 & $23.86$ \\
Intra-var only & 0.0 & 0.0 & 1.0 & 0.0 & $\mathbf{24.43}$ \\
Separability only & 0.0 & 0.0 & 0.0 & 1.0 & $23.71$ \\
\midrule
No separability & 0.25 & 0.25 & 0.50 & 0.0 & --- \\
Equal weights & 0.25 & 0.25 & 0.25 & 0.25 & --- \\
Sep-dominated & 0.10 & 0.10 & 0.20 & 0.60 & --- \\
\midrule
Full \cags{} & 0.3 & 0.3 & 0.2 & 0.2 & $24.42$ \\
\bottomrule
\end{tabular}
\end{table}

The most striking finding is that \emph{intra-class variance alone} ($\gamma{=}1.0$) achieves $24.43\%$---matching the full multi-factor \cags{} ($24.42\%$)---while \emph{inter-class separability alone} ($\delta{=}1.0$) achieves only $23.71\%$, the weakest single-factor variant. This ranking directly correlates with the degree of per-class guidance variation: intra-class variance produces the widest per-class variation (std $0.154$, $\lambda$ range $0.009$--$0.046$), while separability produces near-constant guidance (std $0.006$, $\lambda$ range $0.039$--$0.041$). The near-constant guidance from separability is effectively a fixed guidance at $\lambda \approx 0.04$, which provides some benefit over unguided but cannot exploit per-class complexity differences.

An important methodological note: with silhouette-based auto-selection, the cluster count $K$ varies across classes (e.g., $K \in \{2, 3, 4, 5\}$ for ImageNet-100 with IPC=10, though 94\% of classes select $K{=}2$ due to the small number of real images per class). Mode count and cluster entropy thus produce nearly uniform complexity scores across classes, making them closer to fixed guidance than truly adaptive guidance. Despite this, mode count only ($\lambda \approx 0.013$, near-constant) still improves over unguided ($+2.2\%$ relative), indicating that even very mild fixed guidance helps. However, it underperforms the adaptive variants, confirming that per-class variation provides additional benefit beyond the average guidance level.

Among the weight sensitivity variants, equal weights and no-separability both cluster within a narrow band, suggesting that the specific weight allocation matters less than \emph{including} factors that produce genuine per-class variation. Full \cags{} ($24.42\%$) benefits from its balanced weighting that includes intra-class variance ($\gamma{=}0.2$) alongside the other three factors, producing moderate per-class variation (std $0.035$).
