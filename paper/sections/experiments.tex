...
\Cref{tab:sota-main} presents our main results on ImageNet-100 (100-class, IPC=10) alongside published baselines from the MGD$^3$ paper~\cite{chansantiago2025modeguided}. The unguided baseline achieves $22.79\%$ on ConvNet-6, $26.49\%$ on ResNet-18, and $26.65\%$ on ResNetAP-10. Fixed guidance at $\lambda = 0.10$ yields $21.31 \pm 0.42\%$ on ConvNet-6 ($-6.5\%$ relative), confirming that a globally fixed guidance strength is suboptimal when class complexity varies across 100 classes. The four-factor \cags{} achieves $25.29\%$, $31.14\%$, and $30.70\%$ on the three architectures, while the optimized two-factor configuration $(\alpha{=}0, \beta{=}0.5, \gamma{=}0.5, \delta{=}0)$ achieves $25.70 \pm 0.07\%$, $30.50\%$, and $29.52\%$ with $3.9\times$ lower variance on ConvNet-6. Critically, \cags{} surpasses all published baselines on all three architectures: on ResNet-18, the four-factor achieves $31.14\%$ vs.\ MGD$^3$'s $23.6\%$ ($+7.54\%$ absolute) and IDC-1's $25.1\%$ ($+6.04\%$); on ResNetAP-10, $30.70\%$ vs.\ MGD$^3$'s $25.8\%$ ($+4.90\%$); on ConvNet-6, $25.70\%$ vs.\ MGD$^3$'s $23.4\%$ ($+2.30\%$) and IDC-1's $24.3\%$ ($+1.40\%$). The stronger gains on deeper architectures (ResNet-18: $+15.1$--$17.6\%$ relative vs.\ ConvNet-6: $+10.9$--$12.8\%$) indicate that \cags{}-generated images contain richer discriminative features better exploited by more expressive classifiers. On the 10-class subset, \cags{} is neutral ($-0.7\%$ relative at IPC=10), confirming that per-class adaptation provides no benefit when class complexity variation is limited (Appendix~\ref{sec:results-10cls}).

\begin{table}[t]
\centering
\small
\caption{Main results on ImageNet-100 (100-class, IPC=10). Published baselines (Random--MGD$^3$) are from~\cite{chansantiago2025modeguided}. D4M is our implementation under the same protocol. \cags{} surpasses all published baselines on all three architectures. The four-factor wins on deeper architectures; the two-factor wins on ConvNet-6 with $3.9\times$ lower variance. The \cags{}+D4M hybrid (combining real-image initialization with adaptive guidance) does not improve over \cags{}-alone, confirming that pure-noise generation with adaptive guidance is sufficient.}
\label{tab:sota-main}
\begin{tabular}{lccc}
\toprule
...
\cags{} (4-factor) & $25.29 \pm 0.27$ & $\mathbf{31.14 \pm 0.34}$ & $\mathbf{30.70 \pm 0.07}$ \\
\cags{} (2-factor, optimal) & $\mathbf{25.70 \pm 0.07}$ & $30.50 \pm 0.07$ & $29.52 \pm 0.08$ \\
...
On ConvNet-6, the hybrid peaks at $t_{\text{start}}{=}45$: $25.17 \pm 0.37\%$, compared to $24.27 \pm 0.21\%$ at $t{=}35$ and $23.79 \pm 0.34\%$ at $t{=}40$. At this best operating point, the hybrid is evaluated on all three architectures (\Cref{tab:sota-main}). The results reveal that the hybrid does \emph{not} improve over \cags{}-alone on any architecture: ConvNet-6 $25.17\%$ vs.\ \cags{}'s $25.29$--$25.70\%$ ($-0.12$ to $-0.53\%$), ResNet-18 $30.18\%$ vs.\ $31.14\%$ ($-0.96\%$), and ResNetAP-10 $29.51\%$ vs.\ $30.70\%$ ($-1.19\%$). The gap widens on deeper architectures, suggesting that real-image initialization introduces residual artifacts that constrain the generative prior's expressiveness---precisely the benefit that pure-noise generation with adaptive guidance provides. This finding reinforces the core message: \cags{}'s training-free, pure-noise approach is not only simpler (no real-image access, no $t_{\text{start}}$ hyperparameter) but also more effective than combining real-image initialization with adaptive guidance. The hybrid still substantially outperforms unguided ($+2.38$--$+3.69\%$ absolute) and all published baselines, confirming that the adaptive guidance benefit dominates regardless of initialization strategy.
