...
We evaluate on Food-101~\cite{bossard2014food101} (101 food categories, 1010 training images at IPC=10, 25{,}250 validation images) and CIFAR-10~\cite{krizhevsky2009learning} (10 object categories, 4000 training images at IPC=400, 20{,}000 validation images, upsampled to $256{\times}256$). Food-101 tests generalization to fine-grained visual recognition with large domain gap from ImageNet (food photography vs.\ natural objects). CIFAR-10 tests the boundary condition predicted by our scaling hypothesis: with only 10 classes, the complexity variation is narrow, and \cags{} should be neutral. All experiments use ConvNet-6 with instance normalization, 3 seeds, and 2000 epochs for Food-101, 3000 epochs for CIFAR-10. Text prompts follow the template ``a photo of \{class\_name\}, professional photography'' for Food-101 and ``a photo of a \{class\_name\}'' for CIFAR-10.

\paragraph{Food-101: CAGS generalizes to non-ImageNet domains.}
\Cref{tab:non-imagenet} presents the results. On Food-101, \cags{} achieves $1.81 \pm 0.05\%$ top-1 accuracy compared to $1.51 \pm 0.03\%$ for unguided---an absolute improvement of $+0.30\%$ ($+19.9\%$ relative). This demonstrates that the adaptive guidance principle transfers to a different generator (SD vs.\ DiT) and a different domain (food recognition vs.\ ImageNet objects). The absolute accuracy is low due to the substantial domain gap between SD~1.5's training distribution and Food-101's fine-grained food categories, which limits the fidelity of generated images regardless of guidance strategy. Nevertheless, the $+19.9\%$ relative improvement confirms that per-class complexity adaptation provides meaningful benefit even in this challenging setting, and that \cags{} is not coupled to a specific generative architecture.
...
\begin{table}[t]
\centering
\caption{Non-ImageNet generalization with Stable Diffusion 1.5 as the generator. \cags{} provides a $+19.9\%$ relative improvement on Food-101 (101 classes), demonstrating generalization to a different generator and domain. On CIFAR-10 (10 classes), \cags{} is neutral ($-0.4\%$), confirming the scaling boundary condition. All experiments use ConvNet-6, instance normalization, 3 seeds.}
\label{tab:non-imagenet}
\begin{tabular}{lccccc}
\toprule
\textbf{Dataset} & \textbf{Classes} & \textbf{Generator} & \textbf{Unguided} & \textbf{\cags{}} & \textbf{$\Delta$ (rel.)} \\
\midrule
Food-101 & 101 & SD 1.5 & $1.51 \pm 0.03$ & $\mathbf{1.81 \pm 0.05}$ & $+19.9\%$ \\
