...
\begin{table}[t]
\centering
\caption{Module ablation on ImageNet-100 (100-class) with ConvNet-6, IPC=10, 3 seeds. Best-accuracy tracking per MGD$^3$ protocol. Uses the initial four-factor \cags{} configuration; the simplified two-factor \cags{} achieves $25.70\%$ (Table~\ref{tab:sota-main}). \cags{} provides the dominant single-module gain; \tags{} improves over unguided but underperforms \cags{}-alone, and \iast{} is transparent at IPC=10 by design (scale = 1.0).}
...
\cags{} & $\mathbf{6.51 \pm 0.22}$ & $\mathbf{25.70 \pm 0.07}$ & $\mathbf{41.45 \pm 0.11}$ \\
\iast{}+\cags{} $[0.0, 0.06]$ & $5.87 \pm 0.07$ & --- & --- \\
\midrule
$\Delta$ \cags{} vs.\ unguided (rel.) & --- & $+12.8\%$ & $+19.4\%$ \\
