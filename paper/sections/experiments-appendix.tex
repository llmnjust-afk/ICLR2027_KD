...
\begin{table}[t]
\centering
\caption{IPC ablation on ImageNet-100 (100-class) with ConvNet-6, 3 seeds. Best-accuracy tracking per MGD$^3$ protocol. \cags{} provides consistent and substantial gains at all IPC settings ($+12.4\%$ to $+19.4\%$ relative over unguided). \iast{}+\cags{} \emph{hurts} at IPC=1 by compressing the guidance range.}
...
\cags{} & $\mathbf{6.51 \pm 0.22}$ & $\mathbf{25.55 \pm 0.21}$ & $\mathbf{41.45 \pm 0.11}$ \\
