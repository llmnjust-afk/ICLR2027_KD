...
\begin{table}[t]
\centering
\caption{Comparison with published DD methods on ImageNet-100 (100-class, IPC=10). Published baselines (Random through MGD$^3$) are drawn from the MGD$^3$ paper~\cite{chansantiago2025modeguided}. Our results (Unguided, \cags{}) use the same evaluation protocol (best-accuracy tracking, instance normalization, 3 seeds). Note that our ``Unguided'' baseline generates synthetic images from the frozen DiT without guidance ($\lambda{=}0$), whereas Random and Herding select real images; the two are not directly comparable. \cags{} surpasses all published baselines on all three architectures, including MGD$^3$ itself.}
